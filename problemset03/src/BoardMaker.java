 ...
 */
/*
 * Your parser works great and it was audacious to try to parse it with jParsec.
 * 
 * The contract is well stated, eventhough it could be a bit clearer what happens
 * if there is partially valid input. There is however an obscure bug: enter 
 * "rt 10000000000000000" and it would not be able to handle the int size.
 * 
 * As for the UML, I'm actually not sure, in what program you generated it.
 * There is no standard file format for UML, so please hand it in as an image
 * (e.g. png). A general warning though: more often than not, a generated 
 * UML is gruesome and overloaded. Instead of letting it be generated by
 * some program, we suggest that you design it yourself and - because you wont
 * have a computer on your exam - ideally draw it itself on a paper and take
 * a picture of that, that you can push.
 * 
 * Because you did well on the rest, you shall still...
 * 
 * PASS
 */
class BoardMaker {
